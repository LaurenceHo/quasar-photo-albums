...
export default class AlbumService extends D1Service<any> {
    ...
    override async create(item: any): Promise<any> {
        ...
        if (tags.length > 0) {
            ...
            const tagStmt = this.db.prepare('INSERT OR IGNORE INTO album_tags (tag, createdAt, createdBy) VALUES (?, ?, ?)');
            const tagBatch = tags.map((tag: string) => tagStmt.bind(tag, new Date().toISOString(), item.createdBy || 'system'));
    ...
    override async update(id: string, item: any): Promise<any> {
        ...
        if (tags !== undefined) {
            ...
            if (tags.length > 0) {
                // Ensure tags exist in album_tags table
                const tagStmt = this.db.prepare('INSERT OR IGNORE INTO album_tags (tag, createdAt, createdBy) VALUES (?, ?, ?)');
                const tagBatch = tags.map((tag: string) => tagStmt.bind(tag, new Date().toISOString(), item.updatedBy || 'system'));
                await this.db.batch(tagBatch);
